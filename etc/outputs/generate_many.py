...
if __name__ == "__main__":
    ...
    if not all_args:
        print(
            "The plugin's names whose results will be plotted must be listed explicitely as script arguments."
        )
        sys.exit(1)
    ...
    for PLUGIN_NAME in all_args:
        try:
            ...
            print(
                f"No data found for plugin '{PLUGIN_NAME}'. A file 'results-{PLUGIN_NAME}' must be in this folder."
            )
            sys.exit(1)
        ...
        for plug_name, plug_data in data.items():
            ...
            if eos_data is not None:
                ...
                if BM_fit_data is not None:
                    pl.plot(
                        volumes,
                        np.array(energies) - BM_fit_data["E0"],
                        "o",
                        label=f"{plug_name} EOS data",
                        color=colors[plug_name],
                    )
                else:
            ...
            # Case when eos_data are not present (only BM_fit_data availables), the x range is set around V0
                ...
                dense_volumes = np.linspace(
                    BM_fit_data["min_volume"] * 0.94, BM_fit_data["min_volume"] * 1.06, 100
                )
            ...
            if BM_fit_data is not None:
                ...
                pl.plot(
                    dense_volumes, eos_fit_energy, label=f"{plug_name} fit", color=colors[plug_name]
                )
