...
if __name__ == "__main__":
    ...
    with open("tex-template/results.tex", "w") as fhandle:
        ...
        for element in element_list:
            for configuration in ["XO", "XO2", "XO3", "X2O", "X2O3", "X2O5"]:
                fhandle.write(
                    "\\IfFileExists{../../outputs/plots-%s/%s-%s.png}"
                    % (folder_string, element, configuration)
                )
                fhandle.write(
                    "{\\includegraphics[width=0.15\\linewidth]{../../outputs/plots-%s/%s-%s}}"
                    % (folder_string, element, configuration)
                )
    ...
    print(
        "(Don't forget to rename the file to include the plugin name(s) if you are going to share it)."
    )
