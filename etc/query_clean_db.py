# -*- coding: utf-8 -*-
from aiida.orm import Code, Node, QueryBuilder
...
for node in Group.get(
    label="acwf-verification/unaries-set2/structures/wien2k"
).nodes:  # Giovanni structures
    nodes_to_keep.append(node.id)

for node in Group.get(
    label="acwf-verification/unaries-set2-MPI/structures/wien2k"
).nodes:  # Giovanni structures
    nodes_to_keep.append(node.id)

for node in Group.get(
    label="acwf-verification/unaries-set2-nonMPI/structures/wien2k"
).nodes:  # Giovanni structures
    nodes_to_keep.append(node.id)
